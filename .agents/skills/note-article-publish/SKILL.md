...
`docs/visual-design.md`を読み、全画像の生成・目視検品が同名briefのvisual-reviewに記録されていることを確認する。下書き保存にも適用し、機械チェック通過だけで品質確認済みにしない。

通常はdraft skillのPhase 4で執筆と並行生成済みのPNGを受け取る。ここで生成待ちを発生させない。
完成済み画像がない場合だけagent_type:eyecatch-generator、fork_turns:noneへ
選択したテンプレート画像、確定した記事タイトル、構図に合わせて大きく読みやすく配置する4〜12字目安の短く引きのあるフックコピー、KICKER、雰囲気、drafts/images内の出力先を渡す。フックコピーは記事タイトルの転載・短縮を前提にしない。テンプレート画像がない、またはフックが大きく読めない場合は保存・公開へ進めない。
返却PNGが1280×670pxであることを確認する。画像担当が文字・構図を目視検品する。
同一作業で複数記事を保存する場合、アイキャッチは記事ごとに別プロンプトで生成し、内容ハッシュが重複しないことを確認する。重複時は保存を止めて再生成する。
