...
description: articles/drafts/ にある記事を、Codex-in-Chrome の認証済みブラウザセッションを使って note.com の内部APIに直接fetchし、下書き保存または本公開する。処理はすべて内部APIへの直接リクエストで完結させ、Codex-in-Chromeの画面操作（クリック・スクリーンショット）は使わない。アイキャッチ画像の生成はマストであり、必ず eyecatch-generator agent に委譲して作成し、noteに貼り付ける（設定する）。
---

# note-article-publish

`scripts/note_web_publish.js` を、note.com にログイン済みの Codex-in-Chrome タブ上で
`javascript_tool` により実行し、note の内部API（`https://note.com/api/...`）に直接 `fetch`
して投稿する。**note.comとのやり取りは常にこの内部APIへの直接リクエストで完結させ、
note エディタのUIをクリック操作で動かす方式は使わない**（理由は下記「Codex-in-Chromeの
使用を最小限にする方針」を参照）。
...
実ブラウザセッション（Codex-in-Chrome）**にすべて委ね、投稿処理だけを note の内部APIへの
直接リクエストとして実行する方式にしている。

## Codex-in-Chromeの使用を最小限にする方針

Codex-in-Chromeは「ログイン済みセッションのCookieを使わせるための実行環境」としてのみ使う。
具体的に許可される操作は以下の2つだけ。
...
このスキル自身では一切使わない。アイキャッチ画像は専任の `eyecatch-generator` agentが
テンプレートHTMLを参考に `image_gen` で一から生成する（後述）。**アイキャッチ画像の生成およびnoteへの貼り付け（設定）はマスト（必須）**であり、
必ず生成してnoteに設定する。新規記事の投稿・下書き保存・本公開は、この2操作の組み合わせだけで完結する
設計になっており、note エディタの画面をクリックして操作する必要はない。

## 前提

- Codex-in-Chrome で note.com にログイン済みのタブが存在すること。無ければユーザーに
...
2. Codex-in-Chrome のタブコンテキストを取得し、note.com にログイン済みのタブを特定する
...
**アイキャッチ画像の生成およびnoteへの貼り付け（設定）はマスト（必須）である。**
noteへの下書き保存・本公開を行う際は、必ず事前に `eyecatch-generator` agent
（`.codex/agents/eyecatch-generator.toml`）に、参考テンプレートの選定、`image_gen` による新規生成、
検品、1280×670pxへの仕上げを委譲してアイキャッチ画像を生成し、noteに設定する。HTMLは
配色・構図・文字組みの参考資料であり、コピーやレンダリングには使わない。この工程を独立agentに
切り出しているのは、画像生成のプロンプトや中間生成物でこのスキル自身の文脈を汚さないため
（`AGENTS.md`「スキルとagentの使い分け」参照）。

1. `eyecatch-generator` agentを呼び出す。プロンプトには対象記事の
   ...
   **どの参考テンプレートを使うかは指定しない。** 改行位置・文字サイズ・色の選択は
   agent側の判断に任せ、こちら側では調整しない。
2. agentから返る1280×670pxの最終PNGファイルパスだけを受け取る。生成プロンプトや中間画像は
   返ってこない前提でよい。
3. 受け取ったPNGを base64 化し、次項（画像アップロード）の `eyecatch` に渡す。
4. 今後の生成で参照するデザイン指針を増やしたい場合（配色パターンやレイアウトの別バリエーション等）は、
   `templates/eyecatch_template_*.html` として新しい参考資料を追記・保存する。
...
Codex-in-Chromeの画面操作を最小限にする方針（上記参照）と相容れないため採用しない。
...
- `computer`/`find`/`file_upload` など画面操作系のツールは使わない。アイキャッチ画像作成は
  必ず `eyecatch-generator` agentに委譲し、agentはブラウザを使わず `image_gen` で生成する。
