...
本文・briefを保存し、note-format.mdの一括チェックで計数・目次位置・形式の指摘をまとめて修正する。修正と再チェックの往復は最大3回までとし、3回で解消しない指摘は未解決点として記録し次工程へ進む。
...
具体的修正をすべて反映し、briefのPhase 7に結果・反映箇所を記録する。採点・定例の再監査ループは行わない。修正と再監査の往復は最大3回までとし、3回目でも解消しない指摘は未解決点として明記し公開せず報告する。
